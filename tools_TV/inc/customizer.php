 ...
 */
function tutv_customize_register( $wp_customize ) {
	$color_scheme = tutv_get_color_scheme();
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'tutv_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'tutv' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => tutv_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'tutv' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tutv' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'tutv' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tutv' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tutv' );
}
add_action( 'customize_register', 'tutv_customize_register', 11 );

/**
 * Register color schemes for Twenty Fifteen.
 *
 * Can be filtered with {@see 'tutv_color_schemes'}.
 *
 ...
 */
function tutv_get_color_schemes() {
	return apply_filters( 'tutv_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'tutv' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'tutv_get_color_scheme' ) ) :
/**
 ...
 */
function tutv_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = tutv_get_color_schemes();
...
endif; // tutv_get_color_scheme

if ( ! function_exists( 'tutv_get_color_scheme_choices' ) ) :
/**
 ...
 */
function tutv_get_color_scheme_choices() {
	$color_schemes                = tutv_get_color_schemes();
...
endif; // tutv_get_color_scheme_choices

if ( ! function_exists( 'tutv_sanitize_color_scheme' ) ) :
/**
 ...
 */
function tutv_sanitize_color_scheme( $value ) {
	$color_schemes = tutv_get_color_scheme_choices();
...
endif; // tutv_sanitize_color_scheme
 ...
 */
function tutv_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = tutv_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = tutv_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = tutv_get_color_scheme_css( $colors );

	wp_add_inline_style( 'twentyfifteen-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'tutv_color_scheme_css' );
 ...
 * @since Twenty Fifteen 1.0
 */
function tutv_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', tutv_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'tutv_customize_control_js' );
 ...
 * @since Twenty Fifteen 1.0
 */
function tutv_customize_preview_js() {
...
add_action( 'customize_preview_init', 'tutv_customize_preview_js' );
 ...
 */
function tutv_get_color_scheme_css( $colors ) {
 ...
 * @since Twenty Fifteen 1.0
 */
function tutv_color_scheme_css_template() {
	$colors = array(
	...
	<script type="text/html" id="tmpl-twentyfifteen-color-scheme">
		<?php echo tutv_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'tutv_color_scheme_css_template' );
